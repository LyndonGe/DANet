...
from dataloader import ImageFolder2
from model.san import san
#from model.resnet import resnet34
from model.resnet_add import resnet_dual
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = resnet_dual(pretrained=False, progress=True, num_classes=args.classes)
    #model = torchvision.models.resnet34(pretrained=False, progress=True, num_classes=args.classes)
    criterion = nn.CrossEntropyLoss(ignore_index=args.ignore_label)
    #criterion = FocalLoss(ignore_index=args.ignore_label, class_num = args.classes, gamma=3)
    ...
    mean1, std1 = [0.564, 0.551, 0.550], [0.169, 0.181, 0.187]
    mean2, std2 = [0.329, 0.406, 0.449], [0.288, 0.306, 0.304]
    train_transform1 = transforms.Compose([transforms.Resize((384,384)), transforms.CenterCrop((336,336)), transforms.RandomHorizontalFlip(), transforms.ToTensor(), transforms.Normalize(mean1, std1)])
    train_transform2 = transforms.Compose([transforms.Resize((384,384)), transforms.CenterCrop((336,336)), transforms.RandomHorizontalFlip(), transforms.ToTensor(), transforms.Normalize(mean2, std2)])
    '''
    train_transform = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.RandomHorizontalFlip(), transforms.ToTensor(), transforms.Normalize(mean, std)])
    '''
    #train_set = torchvision.datasets.ImageFolder(os.path.join(args.data_root, 'train'), train_transform)
    train_set = ImageFolder2(os.path.join(args.data_root, 'train-'),os.path.join(args.data_root, 'train+'), train_transform1,train_transform2, aug = True)
    val_transform1 = transforms.Compose([transforms.Resize((384,384)), transforms.CenterCrop((336,336)), transforms.ToTensor(), transforms.Normalize(mean1, std1)])
    val_transform2 = transforms.Compose([transforms.Resize((384,384)), transforms.CenterCrop((336,336)), transforms.ToTensor(), transforms.Normalize(mean2, std2)])
    #val_set = torchvision.datasets.ImageFolder(os.path.join(args.data_root, 'val'), val_transform)
    val_set = ImageFolder2(os.path.join(args.data_root, 'val-'),os.path.join(args.data_root, 'val+'), val_transform1, val_transform2)
    if args.distributed:
