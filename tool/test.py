...
def main():
    ...
    #criterion = FocalLoss(ignore_index=args.ignore_label, class_num = args.classes, gamma=1)
...
def validate(val_loader, model, criterion):
    ...
    logger.info(classification_report(targets, outputs, output_dict=True))
    logger.info('kappa:{}'.format(cohen_kappa_score(targets, outputs)))
    logger.info(confusion_matrix(targets, outputs))
